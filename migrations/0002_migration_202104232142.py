...
def forward(old_orm, new_orm):
    ...
    return [
        # Apply default value False to the field usertable.is_deleted
        usertable.update({usertable.is_deleted: False}).where(usertable.is_deleted.is_null(True)),
    ]


def backward(old_orm, new_orm):
    coupontable = new_orm['coupontable']
    shoptable = new_orm['shoptable']
    usertable = new_orm['usertable']
    return [
        # Check the field `coupontable.description` does not contain null values
        # Apply default value '' to the field shoptable.name
        shoptable.update({shoptable.name: ''}).where(shoptable.name.is_null(True)),
        # Apply default value '' to the field shoptable.phone_number
        shoptable.update({shoptable.phone_number: ''}).where(shoptable.phone_number.is_null(True)),
        # Apply default value '' to the field shoptable.address
        shoptable.update({shoptable.address: ''}).where(shoptable.address.is_null(True)),
        # Apply default value '' to the field shoptable.work_day
        shoptable.update({shoptable.work_day: ''}).where(shoptable.work_day.is_null(True)),
        # Apply default value '' to the field shoptable.city
        shoptable.update({shoptable.city: ''}).where(shoptable.city.is_null(True)),
        # Apply default value True to the field usertable.is_active
        usertable.update({usertable.is_active: True}).where(usertable.is_active.is_null(True)),
        # Apply default value datetime.date(2021, 4, 23) to the field usertable.date_of_birth
        usertable.update({usertable.date_of_birth: datetime.date(2021, 4, 23)}).where(usertable.date_of_birth.is_null(True)),
        # Apply default value '' to the field usertable.phone_number
        usertable.update({usertable.phone_number: ''}).where(usertable.phone_number.is_null(True)),
        # Check the field `usertable.favorite_shop` does not contain null values
        # Apply default value '' to the field usertable.address
        usertable.update({usertable.address: ''}).where(usertable.address.is_null(True)),
    ]
